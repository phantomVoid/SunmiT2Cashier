...
import openapi.request.CardVerBean;
import openapi.request.ZolozAuthenticationCustomerSmilepayInitializeRequest;
import openapi.request.ZolozIdentificationUserWebInitializeRequest;
...
public class AlipaySmileModel {
    ...
    public void getInitClientData(String metainfo, final RequestCallback callback) {
            ...
            public <T extends AlipayResponse> T onResponse(T response) {
                if (response != null && response.isSuccess()) {
                    try {
                        ...
                        callback.onRequestResult("", "网络连接失败");

                    }
                } else {
                    callback.onRequestResult("", "其他错误");
                }
    ...
    public void checkCard(String name, String idNo, String user_id, final RequestCallback callback) {
        ZolozIdentificationUserWebInitializeRequest request
                = new ZolozIdentificationUserWebInitializeRequest();


        request.setBizContent(JSON.toJSONString(buildCardVerRequest(name, idNo, user_id)));
    ...
    private CardVerBean buildCardVerRequest(String name, String idNo, String user_id) {
        CardVerBean cardVerBean = new CardVerBean();
        cardVerBean.biz_id = UUID.randomUUID() + "";
        cardVerBean.identity_param = new CardVerBean.IdentityParamBean();
        cardVerBean.identity_param.identity_type = "CERT";
        cardVerBean.identity_param.cert_type = "IDCARD";
        cardVerBean.identity_param.cert_name = name;
        cardVerBean.identity_param.cert_no = idNo;
        cardVerBean.identity_param.user_id = user_id;
